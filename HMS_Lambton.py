...
# title
title = Label(top_frame, text = "Lambton Hospitals", font = "evogria 45", 
...
def display_patient_details_after_search():
    ...
    resized_profile_pic = random_profile_pic.resize((100, 100), PIL.Image.LANCZOS)
...
def about_window():
    ...
    # main content
    details_1 = str(
            "The Hospital Management System project provides a fully computerized solution for managing operations at the fictional Lambton Hospitals. Through this system, patients can register, review and update their personal information, and explore details about the available doctors and hospital services. Moreover, it facilitates the scheduling of appointments with particular doctors, enhancing the efficiency and convenience of patient interactions with the hospital."
        )

    details_2 = str(
            "This project is created by Raj, Pranav, Preksha, Neehal and Dhaval " +
            "as part of their college Python subject, under " +
            "the able and very helpful guidance of Mehsam Effatti, " +
            "All codes in this file " +
            "are completely written by Raj, Pranav, Preksha, Neehal and Dhaval only. " +
            "All images and icons used are under CC license."
        )
...
def appointment_window():
    ...
    # check patient id
    check_pid = easygui.enterbox(title = "Check PID", msg = "Enter your PID to book Appointment")
...
def urine_test_window():
    ...
    resized_img1 = img1.resize((865, 610), PIL.Image.LANCZOS)
    resized_img2 = img2.resize((865, 610), PIL.Image.LANCZOS)
    resized_img3 = img3.resize((865, 610), PIL.Image.LANCZOS)
    resized_img4 = img4.resize((865, 610), PIL.Image.LANCZOS)
...
def home_submit():
    ...
    resized_main = logo_main.resize((970, 692), PIL.Image.LANCZOS)
...
def create_database_and_table():
    operation = """CREATE DATABASE IF NOT EXISTS hospital"""
    cursor.execute(operation)

    operation = """USE hospital"""
    cursor.execute(operation)
...
def insert_bydefault_data():
    operation = """INSERT INTO patient_details VALUES
					(313570101, "Raj Patel", 17, "M", 8375054875, "Delhi", "O+"),
					(313570102, "Dhaval Sutariya", 17, "M", 7567563156, "Delhi", "O+"),
					(313570103, "Pranav Pal", 18, "M", 8345671848, "Mumbai", "A+"),
					(313570104, "Neehal Hansoti", 16, "M", 7534586798, "Jaipur", "B+")"""
    cursor.execute(operation)
    # dbcon.commit()
...
if (by_default_data_confirm == 1):
    try:
        ...
        speak("Welcome to Lambton Hospitals. How can I help you?")
    except:
